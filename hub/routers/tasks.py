...
def approve_plan(data: dict):
    ...
    # ── Self-execute: send execute message directly to creator (no kanban task) ──
    if plan.get("self_execute", False):
        with lock:
            ...
            # Mark parent task as done (no subtask to track on kanban)
            parent_tid = plan.get("task_id", "")
            if parent_tid and str(parent_tid).isdigit():
                ptid = int(parent_tid)
                if ptid in tasks:
                    tasks[ptid]["status"] = "done"
                    tasks[ptid]["completed_at"] = datetime.now().isoformat()

            # Notify user
            ts = datetime.now().isoformat()
            messages.setdefault("user", []).append({
                "sender": "system", "receiver": "user",
                "content": f"Plan #{plan_id} approved — executing {n_steps} steps directly (no kanban task).",
                "msg_type": "info", "timestamp": ts,
            })

            # Send execute message directly to creator (no kanban task created)
